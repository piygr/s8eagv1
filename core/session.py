...
class MultiMCP:
            ...
            try:
                if config.get('type') in ['sse', 'http']:
                    ...
                    async with sse_client(url) as (read, write):
                        print(f"[agent] Establishing connection, creating session with [{url}]...")
                        await self.fetch_tools(read, write)
                    ...
                    async with stdio_client(params) as (read, write):
                        print(f"[agent] Connection established, creating session [stdio]...")
                        await self.fetch_tools(read, write)
    ...
    async def fetch_tools(self, read, write):
...
class MultiMCP:
    ...
    def get_tool_embedding(self, tool, config):
        response = requests.post(
                        config["tool_embedding_url"],
                        json={"model": config["tool_embedding_model_name"], "prompt": getattr(tool, 'description', 'name')}
                    )
    ...
    async def initialize(self):
        ...
        for config in self.server_configs:
            try:
                if config.get('type') in ['sse', 'http']:
                    url = config.get('url')
                    print(f"→ Scanning tools from: {url}")
                            ...
                            async with ClientSession(read, write) as session:
                                ...
                                for tool in tools.tools:
                                    self.tool_map[tool.name] = {
                                        "config": config,
                                        "tool": tool,
                                        "embedding": self.get_tool_embedding(tool, config)
                                    }
                        except Exception as se:
                            print(f"❌ Session error: {se}")
                else:
    ...
    def get_all_tools(self) -> List[Any]:
        tools = [(entry["tool"], entry["embedding"]) for entry in self.tool_map.values()]
        return tools
