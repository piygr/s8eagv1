...
- User asks: "What’s the relationship between Cricket and Sachin Tendulkar"
  - FUNCTION_CALL: search|query="relationship between Cricket and Sachin Tendulkar"
  - [receives searched results]
  - FUNCTION_CALL: fetch_content|url=https://en.wikipedia.org/wiki/Sachin_Tendulkar
...
- 🚫 Do NOT generate Python code or any `tool_code` blocks.
- 🚫 Do NOT explain or add any extra commentary.
- 🚫 Do NOT invent tools. Use only the available tools listed above. Tool description has useage pattern, only use that.
- 📄 If the question may relate to public/factual knowledge (like companies, people, places), use the `search` tool to search the the answer on web.
- 🧮 If the question is mathematical, use the appropriate math tool.
- If the user intent is to share the results or update it in the spreadsheet, use the data format as desired.
