...
def summarize_tools(tools: List[Any]) -> str:
    ...
    """
    return "\n".join(
        f"- {tool[0].name}: {getattr(tool[0], 'description', 'No description provided.')}"
        for tool in tools
    )
...
def filter_tools_by_hint(tools: List[Any], hint: Optional[str] = None) -> List[Any]:
    """
    If tool_hint is provided (e.g., 'search'),
    ...
    filtered = [tool for tool in tools if hint_lower in tool[0].name.lower()]
...
def get_tool_map(tools: List[Any]) -> Dict[str, Any]:
    ...
    """
    return {tool[0].name: tool for tool in tools}
