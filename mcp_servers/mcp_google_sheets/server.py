...
mcp = FastMCP("Google Spreadsheet", 
              ...
              port=8001, sse_path="/mcp", message_path="/messages/")
...
def update_cells(spreadsheet_id: str,
    ...
    """
    Update cells in a sheet in Google Spreadsheet with a given spreadsheet_id. 
    
    Args:
        spreadsheet_id: The ID of the spreadsheet (found in the URL)
        sheet: The name of the sheet
        range: Cell range in A1 notation (e.g., 'A1:C10')
        data: 2D array of values to update (e.g. [[1, 2], [3, 4], ...])
    
    Returns:
        Result of the update operation

    Usage: 
        update_cells|spreadsheet_id=17a0qgvSEy8GvO_KDNcTORjxQMMQ9hc8_XVAWhPcTcRU|sheet=Sheet1|range=A1:B5|data=[['Nationality', 'Score'], ['NED', '437'], ['GBR', '374'], ['MON', '356'], ['AUS', '292']]
    """
    sheets_service = ctx.request_context.lifespan_context.sheets_service
...
@mcp.tool()
def create_new_spreadsheet(title: str, ctx: Context = None) -> Dict[str, Any]:
    """
    Create a new and empty Google Spreadsheet.
    ...
    Usage:
        create_spreadsheet|title=Country wise score
    """
    sheets_service = ctx.request_context.lifespan_context.sheets_service
...
@mcp.tool()
def create_new_spreadsheet_with_data(title: str, 
                                     range: str,
                                     data: List[List[Any]],
                                     ctx: Context = None) -> Dict[str, Any]:
    """
    Create a new Google Spreadsheet and populate it with input data.
    ...
    Returns:
        Information about the newly created spreadsheet including its ID with results of populating the data in the sheet

    Usage:
        create_spreadsheet|title=Country wise score|range=A1:B5|data=[['Nationality', 'Score'], ['NED', '437'], ['GBR', '374'], ['MON', '356'], ['AUS', '292']]
    """
    sheets_service = ctx.request_context.lifespan_context.sheets_service
    drive_service = ctx.request_context.lifespan_context.drive_service
    folder_id = ctx.request_context.lifespan_context.folder_id
...
def create_sheet(spreadsheet_id: str, 
                ...
                ctx: Context = None) -> Dict[str, Any]:
    """
    Create a new and empty sheet tab in an existing Google Spreadsheet.
